<script lang="ts">
  ...
  let playlist: Track[] = [];
  let currentIndex = 0;
  let filteredTracks: Array<{ track: Track; index: number }> = [];
  ...
  $: filteredTracks = playlist.flatMap((track, index) => {
    const needle = search.trim().toLowerCase();
    const matches = !needle || [track.name, track.artist].some((value) => String(value || '').toLowerCase().includes(needle));
    return matches ? [{ track, index }] : [];
  });
  ...
  function getPanelSize(open = isOpen) {
    if (!open) {
  ...
  function syncPanelToViewport() {
    const { width, height } = getPanelSize(isOpen);
  ...
  function getOpenPositionFromAnchor(fromLeft = panelLeft, fromTop = panelTop) {
    const closed = getPanelSize(false);
    const open = getPanelSize(true);
  ...
  function getClosedPositionFromAnchor(fromLeft = panelLeft, fromTop = panelTop) {
    const closed = getPanelSize(false);
    const open = getPanelSize(true);
  ...
  function getPanelInlineStyle(pLeft: number, pTop: number, open: boolean, aX: string, aY: string) {
    const { width, height } = getPanelSize(open);
  ...
  async function openPlayer(showList = false) {
    ...
    const openPosition = getOpenPositionFromAnchor(closedPosition.left, closedPosition.top);
    panelLeft = openPosition.left;
    panelTop = openPosition.top;
    syncAnchor(panelLeft, panelTop, getPanelSize(true).width, getPanelSize(true).height);
  }
  ...
  async function closePlayer() {
    ...
    const closedPosition = getClosedPositionFromAnchor(panelLeft, panelTop);
    isOpen = false;
    isListOpen = false;
    search = '';
    panelLeft = closedPosition.left;
...
<section aria-labelledby="music-title" class="route-shell music-route-shell">
  <div class="music-route-copy">
    <p class="route-kicker">Music</p>
    <h1 id="music-title">音乐播放器</h1>
    <p>旧播放器能力现已直接承载在 <code>/music</code> 路由中，并继续沿用 <code>/api/music</code> 数据契约。</p>
  </div>
</section>
...
<div
  ...
  style={getPanelInlineStyle(panelLeft, panelTop, isOpen, panelAnchorX, panelAnchorY)}
>
      ...
      <div class="mp-card">
        ...
        >
          ...
          <div class="mp-hero-copy">
            ...
            <h2 id="mp-name" class="mp-title">{currentTrack.name}</h2>
          ...
          <div id="mp-list" class="mp-list-scroll">
            {#if filteredTracks.length > 0}
              {#each filteredTracks as { track, index }}
                <button
                  type="button"
                  class="mp-track-row mp-li {index === currentIndex ? 'is-active' : ''}"
                  on:click={(event) => handleTrackSelect(index, event)}
                >
                  <div class="mp-track-copy">
                    <strong>{track.name}</strong>
                    <span>{track.artist}</span>
                  </div>
                  <small>{index === currentIndex ? (isPlaying ? '正在听' : '已选中') : '播放'}</small>
                </button>
              {/each}
            {:else}
