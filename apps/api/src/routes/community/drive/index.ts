...
import { ensureCommunityCoreSchema, type CommunityAuthEnv, type D1DatabaseLike } from '../../auth';
...
async function getGoogleAuthToken(env: CommunityDriveRuntimeEnv, forceRefresh = false) {
  ...
  const expiresAt = Date.now() + Math.max(60, Number(payload.expires_in || 3600)) * 1000 - 60000;
...
function createDriveRepository(env: CommunityDriveRuntimeEnv): DriveRepository {
  return {
    ...
    async getByIds(input) {
      return await collectEntriesForDeletion(env, input.userId, input.ids);
    },
    ...
    async rename(input) {
      ...
      await env.COMMUNITY_DB.prepare('UPDATE drive_files SET name = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?')
...
export async function deleteDriveEntries(context: DriveRouteContext, body: { ids?: string[]; id?: string }) {
  ...
  const freedBytes = result.deletedItems.filter((entry) => !entry.isFolder).reduce((sum, entry) => sum + Math.max(0, entry.size), 0);
  return {
    ok: true,
